...
func (p *Papaya) HandleSearch(c *gin.Context) {
	...
	err := p.db.DB().Raw("SELECT searches.*, ts_rank(searches.tsv, plainto_tsquery('russian', ?)) as rank FROM searches WHERE searches.tsv @@ plainto_tsquery('russian', ?) OFFSET ? LIMIT ?", q[0], q[0], offset, FeedPagination).Find(&res).Error
	if err != nil {
	...
	err = p.db.DB().Find(&looks, lookIDs).Error
	if err != nil {
		logrus.Errorf("error finding looks by ids: %v", err)
		c.AbortWithStatus(500)
		return
	}
	err = p.db.DB().Find(&topics, topicIDs).Error
	if err != nil {
